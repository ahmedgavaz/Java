...
public class VoucherManager {
    ...
    public static void searchVoucher() {
        ...
        if (type == 1) {
            ...
            for (int i = 0; i < voucherList.size(); i++) {
                if (voucherList.get(i).getVoucherId() == integerAttribute) {
                    isFound = true;
                    list.add(voucherList.get(i));
                }
            }
        }         //if the attribute is double we look in every element which is double
        else if (type == 2) {
            doubleAttribute = Double.parseDouble(attribute);
            for (int i = 0; i < voucherList.size(); i++) {
                if (voucherList.get(i).getVoucherDiscount() == doubleAttribute || voucherList.get(i).getVoucherPrice()==doubleAttribute) {
                    isFound = true;
                    list.add(voucherList.get(i));
                }
            }
            ...
            for (int i = 0; i < voucherList.size(); i++) {
                if (voucherList.get(i).getVoucherType().equals(attribute)) {
                    isFound = true;
                    list.add(voucherList.get(i));
                }
            }
